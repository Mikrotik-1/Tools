(function () {
  ...
  async function runScan(){
    ...
    try { const response=await fetch('/api/doctor/scan',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body),signal:AbortSignal.timeout(60000)}); const contentType=response.headers.get('content-type')||''; if(!contentType.toLowerCase().includes('application/json'))throw Error('خدمة فحص الراوتر غير متاحة على الاستضافة الحالية. يلزم تشغيل خدمة دكتور الشبكة وربطها بالموقع؛ يمكنك استخدام تحليل السجلات يدويًا بالأسفل.'); const result=await response.json(); if(!response.ok)throw Error(result.error||'تعذر الفحص.'); snapshot=result; render(); status('تم الاتصال بالميكروتك واكتمل الفحص.'); }
